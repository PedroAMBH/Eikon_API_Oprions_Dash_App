...
lll=['PETR4I268.SA', 'PETR4H250.SA']
data=optionsInfo[optionsInfo.Instrument.isin(lll)]
data['Position*']=[-1000,1000]
#data['Position*']=np.random.randint(-1,1, size=len(data))
...
for k in range(len(data)):
    if data['Position*'].iloc[k]>=0:
        if "C" in data['PUTCALLIND'].iloc[k]:
            ...
            Rt=Rt*abs(data['Position*'].iloc[k])
            net.append(list(Rt))
        else:
            Rt= data['STRIKE_PRC'].iloc[k] -data['TRDPRC_1'].iloc[k] -Price
            Rt[Rt<=-data['TRDPRC_1'].iloc[k]]=-data['TRDPRC_1'].iloc[k]
...
trace3 = go.Scatter(name="0 line",x=pay.index,
                       ...
                       xaxis = 'x1',yaxis = 'y2',
                       line = dict(color='black', width=2, dash='dash'),fill='tozeroy')

fig.add_trace(trace1, secondary_y=False)
fig.add_trace(trace3, secondary_y=False)

for i,j in zip(days,dists):
    trace2 = ff.create_distplot([j],[str(i)+" - Probabilidade"], bin_size=.5,curve_type='normal',show_hist=False,show_rug=False)
